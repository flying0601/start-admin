...
<script>
import { getAllRolse, updRolse, addRolse, delRolse } from '@/api/roles'
  ...
  methods: {
    ...
    deleteRoles(index) {
      this.$confirm('此操作将永久删除相关数据, 是否继续?', '提示', {
        ...
        .then(() => {
          console.log(this.rolesTab[index], index)
          let param = {
            id: this.rolesTab[index].id
          }
          delRolse(param)
            .then(res => {
              if (res.errno == 0) {
    ...
    editRoles(index, row) {
      ...
      this.$nextTick(() => {
        ...
        // console.log('roq.pGWA', row.pages)
      })
    },
      ...
      this.$refs[form].validate(valid => {
        if (valid) {
          ...
          if (type === 'update') {
            ...
            let param = {
              id: this.rolesTab[index].id,
              username: this.rolesTab[index].key,
              description: this.rolesTab[index].description,
              roles: treeKeys.join(',')
            }
            updRolse(param)
              .then(res => {
